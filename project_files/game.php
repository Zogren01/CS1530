<?php
    ...
    if ( isset( $_POST['submit'] ) ){
        $num = $_REQUEST["assig"];
        $assig = $db->query("SELECT * FROM `$num`;");
        $questions = array();
        $answers = array();
        if($assig->num_rows){
            while($row = $assig->fetch_assoc()){
                array_push($questions, $row['questions']);
                array_push($answers, $row['answers']);
            }
        }
        else{
            echo "Invalid assignment ID <br> Press back arrow to navigate back to previous page";
        }
    }
    echo '<BODY onLoad="startGame()">';
?>
...
const cw = document.body.clientWidth * .9;  //canvas width
...
var subject = "<?php echo $_SESSION['subject'] ?>";  //store value of subject to set map textures based on subject
...
var toClear = "<?php echo count($questions) ?>";
